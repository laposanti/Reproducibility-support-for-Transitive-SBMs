#!/usr/bin/env Rscript
# =============================================================================
# Misspecification experiment: K*=6, kappa=6, psi=0
# Goal: DC-SBM first, WST second, SST third
# =============================================================================
...
out_dir <- file.path("output", "simulation", "raw",
                     paste0("misspec_K6_", format(Sys.time(), "%Y%m%d_%H%M%S")))
...
cat(sprintf("=== Misspecification K=6 study: %s ===\n", Sys.time()))
...
fit_osbm <- function(A, fit_model, z_true, Kt, seed) {
  ...
  cat(sprintf("  Hypers: K_spec=%d  b_kappa=%.1f  gamma=%.3f\n", K_spec, b_kappa, gamma_gn))
