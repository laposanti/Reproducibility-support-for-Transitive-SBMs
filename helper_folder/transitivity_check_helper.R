...
#' Relabel DC-SBM chains using ECR (Equivalence Class Representatives)
#' Handles asymmetric kappa matrices correctly.
# --- UPDATED: Robust DC-SBM Relabeling (Fixes rep(0,K) error) ---
dcsbm_relabel <- function(fit) {
  z_chain <- fit$z
  
  # Safety check for empty/failed chains
  if (is.null(z_chain) || length(dim(z_chain)) < 2) {
    stop("DC-SBM fit returned invalid z_chain")
  }
  
  S <- nrow(z_chain)
  n <- ncol(z_chain)
  
  # Robust Dimension Extraction for Kappa
  # If S=1, R might drop dimensions. We force array structure.
  kappa_raw <- fit$lambda
  ds <- dim(kappa_raw)
  
  # Auto-detect K based on z if kappa dims are weird
  K_z <- length(unique(as.vector(z_chain[1,])))
  
  # Sanitize Kappa Dimensions
  if (length(ds) == 3) {
    K <- ds[2]
  } else if (length(ds) == 2 && S == 1) {
    # S=1 case where it dropped to KxK
    K <- ds[1]
    # Reshape to 1 x K x K
    dim(kappa_raw) <- c(1, K, K) 
  } else {
    # Fallback or Error
    K <- K_z
    warning("Ambiguous kappa dimensions in DC-SBM. Trusting Z.")
  }
  ...
  z_new <- z_chain
  kappa_new <- kappa_raw # S x K x K
  
  # The Error Fix: Ensure K is a scalar integer
  K <- as.integer(K)
  ...
  for (s in 1:S) {
    ...
    # Greedy match logic
    tab <- table(zs, pivot_z)
    perm <- rep(0L, K) # Explicit integer type
    
    present_labels <- as.integer(rownames(tab))
    
    # If present_labels is empty (degenerate), skip
    if (length(present_labels) > 0) {
      for (r in seq_len(nrow(tab))) {
        old_lab <- present_labels[r]
        best_match <- which.max(tab[r, ])
        target_lab <- as.integer(colnames(tab)[best_match])
        perm[old_lab] <- target_lab
      }
    }
    
    # Handle missing labels (assign to self or unused)
    missing_old <- setdiff(1:K, present_labels)
    if (length(missing_old) > 0) {
      perm[missing_old] <- missing_old 
    }
    
    # Apply Map
    z_mapped <- zs
    for(k in 1:K) {
      if(perm[k] != 0) z_mapped[zs == k] <- perm[k]
    }
    z_new[s, ] <- z_mapped
    ...
    if (any(is.na(inv_perm))) {
      # Fallback for messy permutations
      inv_perm[is.na(inv_perm)] <- setdiff(1:K, inv_perm)
    }
    
    # Array subsetting must be robust
    kappa_new[s, , ] <- kappa_raw[s, inv_perm, inv_perm]
  }
  
  list(z = z_new, kappa = kappa_new)
}
